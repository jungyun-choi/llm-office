...
- bridge는 분석 lane과 개발 lane에서 각각 모델 프로세스를 최대 하나씩 실행한다. 따라서 서로 다른 업무의 OpenCode 분석과 Claude 개발은 동시에 실행될 수 있지만, 같은 lane 안에서는 FIFO를 유지한다. 업무는 권한 `0600`의 SQLite에 저장되고, 요청·액션 idempotency와 version 검사를 적용한다. 실행 중 재시작은 자동 게시하지 않고 안전한 실패/재시도 상태로 복구한다. 실패한 요청을 다른 provider 또는 합성 데모로 자동 재전송하지 않는다.
