...
```text
요청
  → Human Gate 0: 오비트 사전 미팅과 확정 브리프
  → OpenCode 분석실 (읽기 전용)
  → Human Gate 1: 분석 패킷 검토
     ├─ 오비트 후속 회의 → 기존 결과 보관 → 같은 Job 재분석
     └─ 사용자 승인 → Claude 개발실
...
사전 미팅은 현재/기대 동작, 레포·DLD·TopView 단서, 완료·테스트 기준 중 요청에 빠진
항목만 최대 세 개 묻는다. 사용자가 확인한 `intakeBrief`는 업무와 함께 SQLite에 저장되고
분석 입력과 Claude coding packet이 같은 내용을 참조한다. 화면에는 모델의 비공개 생각
과정이 아니라 이 짧은 확정 브리프만 표시한다. 이미 구체적인 단순 업무는 `바로 맡기기`로
질문을 건너뛸 수 있으며, 이때 비어 있는 항목은 명시적인 분석팀 확인 사항으로 기록한다.

분석 결과 화면은 좁은 알림 패널이 아니라 본문을 읽는 작업 공간으로 열린다. 결론이나 근거가
부족하면 오비트 후속 회의에서 검토 의견, 추가 자료 경로, 기대 판정 기준을 정리한다. 서버는
현재 분석본과 당시 의견을 이력에 보관하고 같은 Job을 분석 lane의 FIFO 끝에 다시 넣는다.
새 분석은 이전 역할 요약과 사람의 의견을 함께 받으며, 개발 lane은 다른 Job을 계속 처리한다.
...
- 추가 분석에는 최신 `version`, 현재 분석 `runId`, 1~4,000자의 회의 의견이 필요하다.
- 재분석 전 결과는 최대 8개 revision까지 SQLite에 보관하며 결과 보관함에서 다시 열 수 있다.
...
```json
{
  "prompt": "Read buffer를 2MB로 확장해 주세요",
  "executionMode": "auto",
  "intakeBrief": {
    "version": "1",
...
```text
approve_coding
request_reanalysis  analysisRunId=<current run id> feedback=<1~4000자>
