...
- SQLite에 남는 서버 FIFO, 작업 히스토리와 에이전트 오류 표시
- OpenCode 분석실과 Claude 개발실, 두 사무실 사이의 사람 승인 데스크
- 여섯 분석 에이전트와 Claude 구현·테스트·Git 단계의 실시간 상태
- 사내 5+1 순차 분석을 위한 역할별 `대기/작업/검증/완료/실패`, 경과 시간 progress 계약
...
사용합니다. 브리지와 SQLite, OpenCode, Claude 작업은 모두 이 PC에서 순차 실행됩니다.
...
분석실의 여섯 자리는 서로 다른 책임을 가진 **논리 에이전트**입니다. 요청 한 건을 OpenCode
프로세스 1개·모델 턴 1개로 처리하고, 한 번의 구조화 결과에 다섯 전문 역할과 오케스트레이터
결론을 담습니다. 승인 후 Claude Code 프로세스 1개가 별도 worktree에서 구현하며, 서버가
고정된 테스트를 실행합니다. 8GB PC에서도 두 모델 프로세스를 동시에 띄우지 않습니다.
...
- [두 사무실 실행·승인 흐름](docs/DUAL_OFFICE_WORKFLOW.md)
