...
import { CompanyOperationsBoard } from "./company-operations-board";
  ...
  return (
    <section className="office-room office-campus" aria-labelledby="office-floor-title" data-workflow-status={props.focusJob?.state ?? "idle"}>
      ...
      <div className="office-campus__grid">
        ...
        <ClaudeOffice
          job={props.focusJob}
          runtimeLabel={runtime.codingRuntimes?.claude ?? runtime.codingRuntimeLabel}
          opencodeRuntimeLabel={runtime.codingRuntimes?.opencode}
        />
      </div>
...
function getAccessibleJobStatus(job: OfficeJob | null): string {
  ...
  if (job.state === "awaiting_coding_approval") return "분석이 끝났습니다. 개발 파트 배정과 구현 승인이 필요합니다.";
  if (job.state === "awaiting_development_input") {
    const lead = job.developmentPart === "opencode" ? "아르고" : "아틀라스";
    return `개발팀이 사람의 판단을 기다리고 있습니다. ${lead} 질문에 답변하면 개발이 재개됩니다.`;
  }
